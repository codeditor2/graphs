...
EBF3.data = read.table ('/home/rudy/ns-dev/experimentResult/results/congestion-zoom-ndn-EBF3-100-rate-trace.log', header = TRUE, sep = "\t")
EBF3.data$Node = factor (EBF3.data$Node)
EBF3.data$Type = factor (EBF3.data$Type)
EBF3.data = subset (EBF3.data, Type == "InData" & Node == "clientProvider" & FaceId != "0")[,c(1,2,3,7)]
...
names (BestRoute.data) = names (EBF3.data)
...
data = rbind (BestRoute.data,MDBF3.data,EBF3.data)
