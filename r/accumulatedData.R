...
entropyRouting.data = read.table ('../averageInData-100.txt', header = TRUE, sep = "\t")
...
g <- ggplot (data=entropyRouting.data, aes(x=Time, y=AverageInData, group =Type , shape = Type,color = Type,linetype = Type)) +
  ...
  xlab ("Time [second]") +
  ylab ("Accumulated inData number") +

  theme_custom ()


cat ("Writing graph to [100AccumulatedData.pdf]\n")
pdf (file = "extend-AccumulatedData.pdf",width=9,height=6)
g
x = dev.off ();