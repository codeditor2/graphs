...
entropyRouting.data = read.table ('../3averageDropRate.txt', header = TRUE, sep = "\t")
...
g <- ggplot (data=entropyRouting.data, aes(x=Frequency, y=DropRate, group =Type , shape = Type,color = Type,linetype = Type)) +
  ...
  ylab ("Average Drop Data Rate [Kilobytes/s]") +
  ##geom_line()+
...
cat ("Writing graph to [3averageDropRate.pdf]\n")
pdf (file = "3averageDropRate.pdf",width=9,height=6)
g
x = dev.off ();
